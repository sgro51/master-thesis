...
def print_usage_matrix_inversion():
    print("Usage:")
    print("  metos3d [-v] matrixinversion [count] [file-format-in] [file-format-out]")
    print("Example:")
    print("  # create vertical diffusion matrix (multiplied by stepsize) for stepsize 2dt")
    print("  cd data/TMM/2.8/Transport/Matrix5_4/")
    print("  metos3d matrixinversion 12 1dt/Ai2_%02d.petsc 1dt/Di2_%02d.petsc")
########################################################################
...
########################################################################
### create vertical diffusion matrix
########################################################################

# invert_matrix
def invert_matrix(filepathin, filepathout):
    print("Creating vertical diffusion matrix from '%s' to '%s'" % (filepathin, filepathout))
    # check modules
    try:
        import numpy as np
        from scipy.sparse import csr_matrix, eye
    ...
    while i < len(nnzrow):
        if nnzrow[i] != 0:
            ...
            A_inv = np.linalg.inv(csr_matrix.toarray(A[start:end, start:end]))  # invert block matrix
    ...
    # prepare for storage, note: we assume the structure did not change
    matrixarray[0][5] = Dv_csr.indices
    matrixarray[0][6] = Dv_csr.data
    # store matrix
    matrixarray.tofile(filepathout)
...
# dispatch_matrix_inversion
def dispatch_matrix_inversion(m3dprefix, argv):
    # check count
    try:
        matrixcount = int(argv[2])
        # check if positive
        if matrixcount < 1:
            print_error("Matrix count '%s' is not positive." % argv[3])
            print_usage_matrix()
    ...
    try:
        ...
        print_error("No input filepath provided.")
        print_usage_matrix()
        sys.exit(1)
    # check input file format
    try:
        ...
        print_error("No output filepath provided.")
        print_usage_matrix()
        sys.exit(1)
    # convert matrices
    for imat in range(matrixcount):
        ...
        # convert matrix
        convert_matrix(filepathin, filepathout)
